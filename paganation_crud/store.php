...
$name = $email = $password = $gender = $filename = $city = $hobbies =  "";
$validation = [];
...
if (empty($_POST["email"])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $valdation['emailErr'] = "Invalid email format";
    }
}
...
if (empty($_POST["hobbies"])) {
    $validation['hobbiesErr'] = "Gender is required";
} else {
    $hobbies = test_input(implode(',', $_POST["hobbies"]));
}


$sql = "INSERT INTO login_register_user_form (name , email , password , gender , file , city , hobbies)
  VALUES('$name' , '$email' , '$password' , '$gender' , '$filename' , '$city' , '$hobbies')";
